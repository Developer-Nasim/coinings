...
if (game && btnArea && hintBtn && replayBtn) {
    ...
    if (lastBtn != undefined && lastBtn != b) {
      if (b.textContent == lastBtn.textContent) {
        ...
        new TimelineMax({ onComplete: populate })
          .set(
            ".foundTxt",
            { textContent: "Found: " + found, fontWeight: 500 },
            0
          )
          .to(
            ".timePlus",
  ...
  if (timeLeft > 0) {
    ...
    // set coin 
    // sessionStorage.setItem('pass_coin',5);
    ...
    TweenMax.set(".endTxt", {
      textContent: hint + " Try again? " + hint,
      fontSize: 35
    });
    ...
    new TimelineMax()
      ...
      .to(".end", 0.6, { autoAlpha: 1, ease: Power2.easeInOut,display:"block" }, 0)
      .fromTo(
        replayBtn,
        1.0,
  ...
  new TimelineMax()
    ...
    .call(
      function () {
        ...
        TweenMax.set(".timeTxt", { textContent: "20:00" });
        found = 0;
...
let findBlk_eyeCheck = document.querySelector('.the_game.eyes_check')
if (findBlk_eyeCheck) {
  Eye_check()
}